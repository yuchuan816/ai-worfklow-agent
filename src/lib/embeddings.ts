...
export class QwenEmbeddingFunction implements EmbeddingFunction {
  // 必须声明 name 属性
  public readonly name = 'qwen-embedding-function';
    ...
    const embeddings = await Promise.all(
      texts.map(async (text) => {
        const { embedding } = await embed({
          model: ollama.embedding(this.modelName),
          value: text,
        });
        return embedding;
      }),
  ...
  static buildFromConfig(config: QwenEmbeddingConfig): QwenEmbeddingFunction {
    return new QwenEmbeddingFunction(config);
  }
}
